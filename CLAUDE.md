...
- YAML files in `dprr_mcp/context/` are the ontology source of truth: `schemas.yaml` (classes/predicates), `examples.yaml` (SPARQL pairs), `tips.yaml` (pitfalls), `prefixes.yaml` (namespace map). Validation in `validate.py` reads these dynamically — to change the ontology, edit the YAML files, not Python code.
- One-off data scripts live in `scripts/`, not in the package.

## Store

- Data directory follows XDG: `DPRR_DATA_DIR` > `$XDG_DATA_HOME/dprr-mcp` > `~/.local/share/dprr-mcp`.
