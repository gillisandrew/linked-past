...
class DatasetRegistry:
    ...
    def _load_void(self, name: str, dataset_dir: Path) -> None:
        ...
        try:
            ...
            # Sanitize: remove void:uriSpace lines with bare schemes (e.g., "http://")
            # that cause Oxigraph parse errors. These are generated by older VoID builds.
            content = void_path.read_text()
            content = re.sub(r'^\s*void:uriSpace\s+"https?://"\s*;?\s*$', "", content, flags=re.MULTILINE)

            vs = VoidStore()
            vs.load(content, format=RdfFormat.TURTLE, base_iri="http://example.org/")
            void_meta: dict = {}
